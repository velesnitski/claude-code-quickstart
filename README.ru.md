...
![Демо Claude Code в терминале](docs/images/hero-terminal.png)

---

## Как работает Claude (безопасный цикл)

Claude ничего не делает молча. Любое действие проходит через цикл:

```mermaid
flowchart LR
    Gather["Собрать<br/>читает файлы<br/>и CLAUDE.md"] --> Plan["Предложить<br/>показывает<br/>изменение"]
    Plan -- "вы соглашаетесь" --> Act["Действовать<br/>правит, выполняет<br/>команды"]
    Act --> Verify["Проверить<br/>сверяет результат,<br/>замечает ошибки"]
    Verify -.-> Gather
```

Три вещи, которые стоит знать:

- **Без вашего «да» ничего не меняется.** Claude показывает diff и спрашивает разрешение.
- **Plan Mode** (`/plan`) – покажет полный план перед тем, как что-то править. Полезно для крупных изменений.
- **CLAUDE.md** – чем больше контекста в нём, тем меньше вопросов задаст Claude.
